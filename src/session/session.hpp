...
#ifndef SF_SESSION_SESSION_HPP
#define SF_SESSION_SESSION_HPP
...
struct SessionConfig {
    ...
    uint32_t    target_ensembles = 300; ///< Stop early when this many ensembles are decoded (0 = no limit)
};
...
struct SessionStats {
    ...
    double   avg_rate_hz     = 0.0; ///< Average notification rate in Hz
};
...
private:
    ...
    static constexpr size_t QUEUE_CAPACITY = 128; ///< Maximum queued packets before dropping

    /// Pimpl keeps @c <thread> and @c <mutex> out of this header.
    struct Impl;
    Impl* impl_;
};
