...
/**
 * @brief Private session state shared between the BLE callback and worker thread.
 *
    ...
    while (true) {
        RawPacket pkt{};
        {
            ...
            impl_->cv.wait(lock, [this] {
                return !impl_->queue.empty() || impl_->stop.load();
            });
...
void Session::dispatch(const sf::protocol::DecodedEnsemble& ens) {
    std::visit([this](const auto& sample) {
...
bool Session::run() {
    ...
    std::cout << "[session] Streaming for " << cfg_.run_duration_s << " s"
              << (cfg_.target_ensembles
                      ? " (or " + std::to_string(cfg_.target_ensembles) + " ensembles)"
                      : "")
              << "\n";
    ...
    stats_.avg_rate_hz = total > 0.0
        ? static_cast<double>(stats_.notify_count) / total : 0.0;
