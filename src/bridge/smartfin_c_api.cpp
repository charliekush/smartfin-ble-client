...
struct BridgeSink final : sf::pipeline::ISampleSink {
    ...
    void on_imu(const sf::protocol::DecodedImu& s) override          { imu.push_back(s); }
    void on_quat_imu(const sf::protocol::DecodedQuatImu& s) override  { quat_imu.push_back(s); }
    void on_temperature(const sf::protocol::DecodedTemp& s) override  { temps.push_back(s); }
    void on_fw_version(const sf::protocol::DecodedFwVersion&) override {}

    void clear() { imu.clear(); quat_imu.clear(); temps.clear(); }
};
...
int sf_sink_push_packet(SF_Sink* s, const uint8_t* data, size_t len) {
    ...
    if (!sf::protocol::decode_packet(std::span{data, len}, ensembles)) return -1;
...
size_t sf_sink_imu_count(const SF_Sink* s)      { return s ? s->sink.imu.size()      : 0; }
size_t sf_sink_quat_imu_count(const SF_Sink* s) { return s ? s->sink.quat_imu.size() : 0; }
size_t sf_sink_temp_count(const SF_Sink* s)     { return s ? s->sink.temps.size()    : 0; }

void sf_sink_get_imu(const SF_Sink* s, size_t i, SF_Imu* out) {
    if (!s || !out || i >= s->sink.imu.size()) return;
...
void sf_sink_get_quat_imu(const SF_Sink* s, size_t i, SF_QuatImu* out) {
    if (!s || !out || i >= s->sink.quat_imu.size()) return;
...
void sf_sink_get_temp(const SF_Sink* s, size_t i, SF_Temp* out) {
    if (!s || !out || i >= s->sink.temps.size()) return;
...
SF_OrientedSample* sf_proc_run(SF_Proc* p, SF_Sink* s, size_t* out_count) {
    if (!p || !s || !out_count) return nullptr;
    *out_count = 0;
